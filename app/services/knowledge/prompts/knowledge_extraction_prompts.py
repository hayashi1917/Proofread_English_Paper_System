...
- **ナレッジの記述形式**:
    各ナレッジのknowledgeフィールドには、以下の形式で詳細な情報を箇条書きで記載すること:
    
    - ナレッジ: [具体的なルール・指示内容]
    - 背景: [なぜこのルールが必要なのか]
    - 使われるべき状況: [どのような場面で適用するか]
    - 例: [具体的な使用例やコマンド例]

- **記述例**:
    knowledge: "- ナレッジ: 図のキャプションは図の下に配置
    - 背景: 読者が図とその説明を一続きとして理解するため
    - 使われるべき状況: 論文に図表を挿入する際
    - 例: \\caption{{実験結果のグラフ}}を\\end{{figure}}の直前に配置"

# 制約
* 上記「ナレッジの定義と抽出基準」を厳密に遵守し、**特に「抽出対象外」に該当する情報は絶対に含めない**こと。
* 抽出するナレッジは、論文の**英文校正および内容・体裁のフォーマット適合に直接役立つ具体的な指示・ルールのみ**とする。
* ナレッジは、その指示内容が論文の体裁や内容にどう影響するかを日本語で平易に表現するか、指示内容に直接関わらない場合は除去する。
